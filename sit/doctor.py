...
def build_doctor_payload(package_spec: str | Path = ".") -> dict[str, Any]:
    ...
    checks.append(_check_git_hooks(Path(package_spec).expanduser()))
...
def _check_git_hooks(package_path: Path) -> DoctorCheck:
    root = git_root(_existing_path(package_path))
    if root is None:
        return DoctorCheck("git_hooks", "pass", "skipped (not a git repo)")

    hook_ref = _git_output(root, ["rev-parse", "--git-path", "hooks/pre-commit"])
    hook_path = Path(hook_ref)
    ...
    if not hook_path.exists():
        return DoctorCheck(
            "git_hooks",
            "warn",
            "sit pre-commit hook is not installed",
            ["run: sit install-hooks ."],
        )

    hook_text = hook_path.read_text(encoding="utf-8", errors="replace")
    if "Generated by sit install-hooks" not in hook_text:
        return DoctorCheck(
            "git_hooks",
            "warn",
            "pre-commit hook exists but is not managed by sit",
            [str(hook_path), "run: sit install-hooks . --force, or add sit checks to the existing hook manually"],
        )

    return DoctorCheck("git_hooks", "pass", "sit pre-commit hook installed", [str(hook_path)])
